...
function toggleUI() {
  ...
  if (secondValEl)
    secondValEl.style.display =
      state.action !== "conversion" ? "block" : "none";
  if (toGroupEl)
    toGroupEl.style.display = state.action === "conversion" ? "block" : "none";
...
function attachEventListeners() {
  ...
  if (fromSelect)
    fromSelect.addEventListener(
      "change",
      (e) => (state.fromUnit = e.target.value),
    );
  if (toSelect)
    toSelect.addEventListener("change", (e) => (state.toUnit = e.target.value));
  if (secondSelect)
    secondSelect.addEventListener(
      "change",
      (e) => (state.secondUnit = e.target.value),
    );
  if (operatorDropdown)
    operatorDropdown.addEventListener(
      "change",
      (e) => (state.operator = e.target.value),
    );
