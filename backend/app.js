...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function attachEventListeners() {
    ...
    actionButtons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        state.action = (button.textContent || "conversion")
          .trim()
          .toLowerCase();
  ...
  async function loadUnits(type) {
    ...
    const allUnits = Array.isArray(unitsPayload)
      ? unitsPayload
      : unitsPayload?.units;
    ...
    const matchingUnits = allUnits.filter(
      (unit) => String(unit.type).toLowerCase() === type.toLowerCase(),
    );
  ...
  function toggleOperators(show) {
    const operatorRow =
      document.querySelector(".operator-row") ||
      document.querySelector("[data-operator-row]");
  ...
  async function loadHistory() {
    ...
    const items = Array.isArray(historyPayload)
      ? historyPayload
