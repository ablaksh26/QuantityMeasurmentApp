  ...
  convertBtn.addEventListener("click", async () => {
    ...
    if (state.action === "arithmetic") {
      ...
      try {
        ...
        const result = performArithmetic(state.fromVal, v2, state.operator);

        document.querySelector("#comparison-result").textContent =
          `Result: ${result} ${state.fromUnit}`;
        ...
        document.querySelector("#comparison-result").textContent = err.message;
      }
    }
  });
}
...
function toggleOperators(show) {
  document.querySelector(".operator-row").style.display = show
    ? "flex"
    : "none";
  document.querySelector(".to-group").style.display = show ? "none" : "block";
